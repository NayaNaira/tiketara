...
@section('title', 'Kelola Buyer')

@section('content')
<header class="p-4 lg:p-8 pb-2 lg:pb-4">
    <h1 class="text-2xl font-bold text-[#C9A84C] mb-1">Kelola Buyer</h1>
    <p class="text-sm text-[#4A9FD4]">Manajemen akun buyer, penangguhan, pemblokiran, dan penghapusan akun.</p>
</header>
        ...
        <table class="w-full text-left text-sm whitespace-nowrap min-w-max">
            <thead class="text-[#4A9FD4] border-b border-[#4A9FD4]/30">
                <tr>
                    ...
                    <th class="px-6 py-4 font-medium text-center">Transaksi Pembelian</th>
                    ...
                    <tr class="hover:bg-white/5 transition">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                @if($buyer->avatar)
                                    <img src="{{ $buyer->avatar }}" class="w-10 h-10 rounded-full object-cover border border-[#C9A84C]/50">
                                @else
                                    <img src="https://ui-avatars.com/api/?name={{ urlencode($buyer->name) }}&background=C9A84C&color=fff" class="w-10 h-10 rounded-full object-cover">
                                @endif
                                <div>
                                    <div class="font-medium text-white">{{ $buyer->name }}</div>
                                    <div class="text-xs text-gray-400">{{ $buyer->email }}</div>
                                </div>
                            </div>
                        </td>
                        <td class="px-6 py-4 text-[#DADADA]">{{ $buyer->phone_number ?? '-' }}</td>
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <!-- Action Suspend -->
                                <form action="{{ route('super.buyers.suspend', $buyer->id) }}" method="POST" class="inline m-0">
                                    @csrf
                                    ...
                                    @else
                                        <button type="submit" class="px-3 py-1.5 bg-yellow-600/10 text-yellow-500 border border-yellow-500/30 rounded hover:bg-yellow-500 hover:text-black transition text-xs font-semibold cursor-pointer" onclick="return confirm('Apakah Anda yakin ingin menangguhkan akun {{ $buyer->name }}?')">
                                            Suspend
                                        </button>
                                    @endif
                                </form>
                                ...
                                <!-- Action Ban -->
                                <form action="{{ route('super.buyers.ban', $buyer->id) }}" method="POST" class="inline m-0">
                                    @csrf
                                    ...
                                    @else
                                        <button type="submit" class="px-3 py-1.5 bg-red-600/10 text-red-500 border border-red-500/30 rounded hover:bg-red-500 hover:text-white transition text-xs font-semibold cursor-pointer" onclick="return confirm('Apakah Anda yakin ingin memblokir (Ban) akun {{ $buyer->name }}?')">
                                            Ban
                                        </button>
                                    @endif
                                </form>
                                ...
                                <!-- Action Delete -->
                                <form action="{{ route('super.buyers.destroy', $buyer->id) }}" method="POST" class="inline m-0">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="px-3 py-1.5 bg-red-600 text-white rounded hover:bg-red-700 transition text-xs font-semibold cursor-pointer" onclick="return confirm('PERINGATAN: Menghapus akun {{ $buyer->name }} akan menghapus seluruh data transaksi pemesanan tiket yang terkait secara permanen! Apakah Anda yakin?')">
                                        Hapus
                                    </button>
                                </form>
                            </div>
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-10 text-center text-gray-500 text-sm">
                            Tidak ada data buyer yang terdaftar.
                        </td>
                    </tr>
                @endforelse
            </tbody>
